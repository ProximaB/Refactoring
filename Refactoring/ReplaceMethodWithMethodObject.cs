namespace Refactoring
{
	public class ReplaceMethodWithMethodObject
	{
		...
		private class Gamma
		{
			...
			public int Compute() => ImportantThing(GetImportantValue2()) * 7 - 2 * GetImportantValue1();

			private int ImportantThing(int importantValue2)
			{
				if (_yearToDate - GetImportantValue1() > 100) importantValue2 -= 20;
				return importantValue2;
			}

			private int GetImportantValue2() => _inputVal * _yearToDate + 100;

			private int GetImportantValue1() => _inputVal * _quantity + Delta();
